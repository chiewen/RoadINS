...
void execute(const vector<PNode> &nodes, vector<Trajectory> &trajectories, int k, int x, int step) {
    ...
    print_result("VR", mps.begin(), mps.begin() + 6);
    print_result("VD", mps.begin() + 6, mps.begin() + 12);
    print_result("IR", mps.begin() + 12, mps.begin() + 18);
    print_result("ID", mps.begin() + 18, mps.end());
    cout << endl;
}
...
TEST(Experiment, First) {
    ...
    copy(trajectories.begin(), trajectories.begin() + 12, back_inserter(trajectories));
